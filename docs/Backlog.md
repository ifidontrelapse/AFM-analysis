...
| B-063 | **`estimate_rough_radius` truncates its own estimate with `int()` before rounding it** | candidate — needs its own task | `radius_px = int(np.sqrt(median_area / np.pi))` is a **second, undeclared rounding** in a function whose only rounding is supposed to be `_integer_radius` (ADR-0020), and it is the same `int()` pattern ADR-0024 deleted as D-04's mechanism. It is also how the estimate reached exactly **0** rather than 0.96 (B-061 / M3-T23, which fixed the consequence and left the cause). Measured effect of deleting it, rough radius before → after: `afm_flat_monodisperse` **14 → 15**, `afm_tilted_polydisperse` **12 → 14**, `afm_dense_overlapping` **11 → 12**, `afm_coarse_pixels` **7 → 9**, `afm_sparse_low_snr` **3 → 3**. That propagates to the final opening radius, the substrate and every height, so it needs its own ADR, its own golden update and a real review of whether the `× 1.7` scale factor was tuned around the truncation. |
