...
| B-058 | **The golden records CPython exception messages verbatim, so it is pinned to the interpreter's minor version** | candidate | `capture.py:_record` stores `str(exc)[:300]`, and `degenerate_inputs` compares it exactly. CPython 3.14 reworded `too many values to unpack (expected 2)` to `… (expected 2, got 3)`, which the first real CI run reported as characterization drift — a red gate with no scientific change behind it (M1-T08). Pinned to 3.12 in CI as the immediate fix, but the fragility stands: **upgrading Python will look like drift**, and M2 rests on "zero drift" holding across machines. Options: compare `error_type` exactly and the message loosely; normalise known rewordings; or record messages but exclude them from comparison. All three change what the harness records, so this needs an ADR (PROJECT_RULES §4.3), not a quiet edit. Trigger: before any Python upgrade, and before M2 if it starts on a second machine. |
