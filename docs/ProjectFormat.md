...
| What are the measurements? | the file the run points at, under `results/` — the table is variable by construction (ADR-0031), so the database indexes it rather than holding it (ADR-0042). **What each column means** is `docs/Measurements.md` (M7-T10) |
| What did the operator measure by hand? | the database — `rulers` (schema v8). A line has no area, so it is not an annotation; and `measurements.csv` already names what an *analysis run* produces, so the hand-drawn one gets its own word (ADR-0074). The **length is not stored** — it is arithmetic over the two endpoints |
| What did the operator draw? | the database — `annotations`. Hand-made, edited one at a time, and the one thing here that cannot be recomputed (ADR-0044). A row carries a **box**, and since schema v6 an optional **outline** beside it: `points IS NULL` is a box drawn as a box, and a polygon's `x1…y2` are its derived bounding box, so every reader that consumes boxes keeps working (ADR-0072). `annotations/` holds **painted masks**, which are files: schema v7 keeps `mask_path` on the row and the PNG under `annotations/mask_<id>.png`, because an array the size of a scan is not a database column (PROJECT_RULES §5, ADR-0073) |
