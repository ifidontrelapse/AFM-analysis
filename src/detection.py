...
class BaseDetector(ABC):
    ...
    def _blobs_to_detections(blobs: np.ndarray) -> list[Detection]:
        """
        Convert a blobs array (N, 4) [y, x, sigma, radius_nm] to a list of Detection.
        Used internally by LogDetector.
        """
        detections = []
        for blob in blobs:
...
class YoloDetector(BaseDetector):
    ...
    def _scale_boxes(self, boxes: np.ndarray, original_shape: tuple) -> np.ndarray:
        ...
        h, w = original_shape
        scale = np.array([
            w / self.yolo_size, h / self.yolo_size,
            w / self.yolo_size, h / self.yolo_size,
        ])
        return boxes * scale
    ...
    def _boxes_to_detections(
        boxes: np.ndarray, pixel_size_nm: float
    ) -> list[Detection]:
        detections = []
        for box in boxes:
            x1, y1, x2, y2 = box
